...
Hardware requirements:
 ...
 - Several Neopixels, you can mix and match
   ...
   o Adafruit NeoPixel Digital LED leftstrip (ID: 1138)
...
#define LEFTLED_PIN    0  // The left NeoPixel LED strand is connected to GPIO #0 / D0
#define RIGHTLED_PIN   1  // The right NeoPixel LED strand is connected to GPIO #1 / D1
...
Adafruit_NeoPixel  leftstrip = Adafruit_NeoPixel(N_PIXELS, LEFTLED_PIN, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel  rightstrip = Adafruit_NeoPixel(N_PIXELS, RIGHTLED_PIN, NEO_GRB + NEO_KHZ800);
...
#endif
  leftstrip.setBrightness(bright);    // Set LED brightness (if POT_PIN at top
                                  //  define commented out, will be full)
  // Color leftstrip pixels based on rainbow gradient
  for(i=0; i<N_PIXELS; i++) {  
    if(i >= height)               
       leftstrip.setPixelColor(i,   0,   0, 0);
    else 
       leftstrip.setPixelColor(i,Wheel(map(i,0,leftstrip.numPixels()-1,30,150)));
    } 

   leftstrip.show(); // Update leftstrip

  // Color rightstrip pixels based on rainbow gradient
   for(i=0; i<N_PIXELS ; i++) {  
    if(i >= height)               
       rightstrip.setPixelColor(rightstrip.numPixels() - i,   0,   0, 0);
    else 
       rightstrip.setPixelColor(rightstrip.numPixels() - i ,Wheel(map(i,0,rightstrip.numPixels()-1,30,150)));
    } 

   rightstrip.show(); // Update rightstrip
